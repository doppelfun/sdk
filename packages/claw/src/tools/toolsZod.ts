...
export const startConversationSchema = z.object({
  targetSessionId: z.string().describe("Session id of the player/NPC/agent to start a conversation with (from get_occupants)."),
  openingMessage: z.string().optional().describe("Optional first message to send; if omitted, only sets the conversation target for the next chat."),
});
...
export const CLAW_TOOL_REGISTRY: Array<{
  ...
  },
  {
    ...
    description:
      "Start a conversation with another player, NPC, or agent. Pass targetSessionId (from get_occupants). Optionally pass openingMessage to send the first message; otherwise only sets the conversation target for the next chat.",
  ...
  // },
  {
    ...
    description:
      "Build a scene using code (Python sandbox via Google Gemini). Use when the user asks to build something complex, mathematical or mentions 'using code', 'with code', 'programmatic', or 'with Python'. Same args as build_full (instruction, optional documentTarget/documentId). Requires LLM_PROVIDER=google and GOOGLE_API_KEY. If it fails, use build_full instead.",
    schema: buildFullSchema,
  },
  {
