 ...
 * and auto-tops-up from the Privy wallet when balance drops below threshold.
 */
 ...
 * and purchases more via crypto when it drops below threshold.
 */
export function startCreditMonitor(
  ...
  const {
    ...
    privyAppSecret,
    privyWalletId,
    privySessionPrivateKey,
    openRouterApiKey,
    creditTopUpThresholdUsd,
    creditTopUpAmountUsd,
  } = config;

  if (!privyAppId || !privyAppSecret || !privyWalletId || !privySessionPrivateKey) {
    return;
  }
  ...
  const check = async () => {
    ...
    if (balance < creditTopUpThresholdUsd) {
      purchasing = true;
      onLog?.(`[credit-monitor] Balance below $${creditTopUpThresholdUsd}, purchasing $${creditTopUpAmountUsd} in credits...`);

      const result = await purchaseCredits({
        privyClient: privy,
        walletId: privyWalletId,
        sessionPrivateKey: privySessionPrivateKey,
        amountUsd: creditTopUpAmountUsd,
        openRouterApiKey,
      });

      if (result.ok) {
        onLog?.(`[credit-monitor] Purchased $${creditTopUpAmountUsd} in credits (${result.creditsAdded} credits added)`);
      } else {
